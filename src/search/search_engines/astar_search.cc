...
AstarSearch::AstarSearch(const Options &opts)
    : SearchEngine(opts),
      iteration(0),
      closed_list(opts.get<shared_ptr<ClosedList>>("closed")),
      open_node_registry(OpenAstarNode::no_node),
      open_state_registry(task_proxy) {
    create_open_list_and_f_eval(opts);
    if (opts.get<shared_ptr<Evaluator>>("eval")->does_cache_estimates()) {
...
static shared_ptr<SearchEngine> _parse(OptionParser &parser) {
    ...
    parser.add_option<shared_ptr<ClosedList>>(
        "closed",
    ...
    return make_shared<astar_search::AstarSearch>(opts);
}

static Plugin<SearchEngine> _plugin("astarmod", _parse);
}