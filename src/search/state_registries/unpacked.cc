...
void UnpackedStateRegistry::print_statistics(utils::LogProxy &log) const {
    ...
    log << "Integers per DB entry (No lookup structure): " << num_variables << endl;
    log << "Number of DB entries: " << size() << endl;
    ...
    utils::g_log << "State set destroyed, size per entry: " << num_variables << " integers"<< std::endl;
