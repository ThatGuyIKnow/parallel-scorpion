#include "htree_packed_state_registry.h"
...
HTreePackedStateRegistry::HTreePackedStateRegistry(const TaskProxy &task_proxy)
...
StateID HTreePackedStateRegistry::insert_id_or_pop_state() {
    return StateID(0);
}

State HTreePackedStateRegistry::lookup_state(StateID id) const {
    std::vector<uint32_t> buffer = tree_table.lookup(id.value);
...
State HTreePackedStateRegistry::lookup_state(
    StateID id, vector<int> &&state_values) const {
    return task_proxy.create_state(*this, id, move(state_values));
}

const State &HTreePackedStateRegistry::get_initial_state() {
...
State HTreePackedStateRegistry::get_successor_state(const State &predecessor, const OperatorProxy &op) {
    assert(!op.is_axiom());
...
int HTreePackedStateRegistry::get_state_size_in_bytes() const {
    return get_bins_per_state() * sizeof(uint32_t);
}

int HTreePackedStateRegistry::get_bins_per_state() const {
    return state_packer.get_num_bins();
}

void HTreePackedStateRegistry::print_statistics(utils::LogProxy &log) const {
