...
void* RTrealloc(void *rt_ptr, size_t size){
	if (size==0) { // macosx realloc(??.0) does not return nullptr!
		free(rt_ptr);
		return nullptr;
	}
	void *tmp=realloc(rt_ptr,size);
	if (tmp==nullptr) printf("out of memory trying to resize to %zu",size);
	return tmp;
}
...
void* RTmalloc(size_t size){
	...
	if (tmp==nullptr) printf("out of memory trying to get %zu",size);
	return tmp;
}
...
TreeFold_ret(treedbs_t dbs,int *vector, int *idx)
{
    ...
    if (nPars==1) {
        ...
        if(vector[0]>=dbs->range){
            ...
            dbs->map=(int*)RTrealloc(dbs->map,dbs->range*sizeof(int));
            dbs->rev=(int*)RTrealloc(dbs->rev,dbs->range*sizeof(int));
            while(old<dbs->range){
                dbs->map[old]=-1;
                dbs->rev[old]=-1;
                old++;
...
int TreeDBSContains(treedbs_t dbs, int *vector) {
	int nPars = dbs->nPars;
	if (nPars == 1) {
		// For nPars == 1, simply check if the value exists in the map
		if (vector[0] >= dbs->range || dbs->map[vector[0]] == -1) {
			return -1; // Not found
		}
		return dbs->map[vector[0]];
	} else {
		// For nPars > 1, traverse the tree without inserting
		int tmp[nPars];
		for (int i = nPars - 1; i > 0; i--) {
			int left = dbs->db_tree_left[i];
			int right = dbs->db_tree_right[i];
			int left_val = (left < nPars) ? tmp[left] : vector[left - nPars];
			int right_val = (right < nPars) ? tmp[right] : vector[right - nPars];

			// Lookup in the hash table for the current node
			int hash = hashvalue(left_val, right_val) & dbs->db_mask[i];
			int result = dbs->db_hash[i][hash];
			while (result >= 0) {
				if (dbs->db_left[i][result] == left_val && dbs->db_right[i][result] == right_val) {
					tmp[i] = result;
					break;
				}
				result = dbs->db_bucket[i][result];
			}
			if (result < 0) {
				return -1; // Not found
			}
...
static void TreeInfoPrint(int depth,int node,treedbs_t dbs){
	...
	prefix[2*depth]=0;
	printf("%s%d: node has %d elements and %d hash slots",prefix,node,dbs->db_next[node],dbs->db_hash_size[node]);
...
void TreeInfo(treedbs_t dbs){
	if (dbs->nPars==1){
		printf("map with %d entries and range of %d elements.",dbs->count,dbs->range);
	} else {
