...
if project.REMOTE:
    ENV = project.TetralithEnvironment(email="hugax961@student.liu.se", extra_options="#SBATCH -A snic2022-5-341",memory_per_cpu="2G")
    ...
    SUITE = ["depot:p01.pddl","depot:p02.pddl", "grid:prob01.pddl", "gripper:prob01.pddl", "gripper:prob02.pddl", "gripper:prob03.pddl", "gripper:prob04.pddl", "gripper:prob05.pddl",
            "airport:p01-airport1-p1.pddl", "airport:p04-airport2-p1.pddl", "airport:p03-airport1-p2.pddl", "airport:p05-airport2-p1.pddl", "airport:p06-airport2-p2.pddl", "airport:p07-airport2-p2.pddl", "airport:p08-airport2-p3.pddl", "airport:p09-airport2-p4.pddl", "airport:p10-airport3-p1.pddl", "airport:p11-airport3-p1.pddl", "airport:p12-airport3-p2.pddl", "airport:p13-airport3-p2.pddl", "airport:p14-airport3-p3.pddl", "airport:p15-airport3-p3.pddl", "airport:p16-airport3-p4.pddl"]
...
for config_nick, config in CONFIGS:
    for rev, rev_nick in REVS:
        algo_name = f"{rev_nick}:{config_nick}" if rev_nick else config_nick
# pairs = [
#     ("origin/master:01-no-closed-blind", "origin/master:02-loes-blind"),
#     ("origin/master:01-no-closed-blind", "origin/master:03-cloes-blind"),
#     ("origin/master:01-no-closed-blind", "origin/master:04-bdd-blind"),
# ]
pairs = [
    ("-no-closed-", "-loes-"),
    ("-no-closed-", "-cloes-"),
    ...
    for form in ["tex", "png"]:
        exp.add_report(
            project.ScatterPlotReport(
                ...
                filter=[project.add_proportion_closed, project.add_evaluations_per_time, attr_comparison_filter("-cloes-", "-bdd-", attr1, attr2)],
                title=None if form == "tex" else f"{attr1} vs {attr2}",
                xlabel=None if form == "tex" else attr1,
                ylabel=None if form == "tex" else attr2,
                format=form,
            ),
            name=f"{exp.name}-{attr1}-vs-{attr2}{form}",
        )
