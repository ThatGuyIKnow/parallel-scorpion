...
PROPORTION_CLOSED = Attribute(
    "proportion_closed", min_wins=False, function=arithmetic_mean, digits=5
)

PROPORTION_MAX_CLOSED = Attribute(
    "proportion_max_closed", min_wins=False, function=arithmetic_mean, digits=5
)
...
def add_proportion_closed(run):
    closed = run.get("closed_states")
    open = run.get("open_states")
    if open is not None:
        if closed is None:
            if open > 0:
                run["proportion_closed"] = 0
        elif (open + closed) > 0:
            run["proportion_closed"] = closed / (closed + open)
    max_open = run.get("max_open_states")
    if max_open is not None:
        if closed is None:
            if max_open > 0:
                run["proportion_max_closed"] = 0
        elif (max_open + closed) > 0:
            run["proportion_max_closed"] = closed / (closed + max_open)
    return run
